...
Tiene una propiedad pública `$tenant`, que contiene una instancia de `EsInquilino` (generalmente `Eddwar\Multitenencia\Models\Inquilino`, o tu modelo personalizado si lo has configurado).
...
Este evento se disparará cuando el método `buscarParaPeticion()` del `BuscadorDeInquilinos` no encuentre ningún inquilino para la solicitud dada.
